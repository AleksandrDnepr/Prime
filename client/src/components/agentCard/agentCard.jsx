...
export class AgentCard extends Component {
    ...
    async sendMail(info, email, id) {
        await  fetch(`api/agents/${id}/send-mail`, {
            method: 'POST',
            headers: {
...
statusForm() {
    const { status, email, id } = this.props;
    ...
    case "default": 
        return <ContactForm onSubmit={info => this.sendMail(info, email, id)}/>;
    default:
        break;
}
}
